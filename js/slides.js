...
Navigate to the "Settings" section below to modify the script behavior.
...
// Add browser and OS information as <html> tag classes:
 ...
 *                                                  
 *     Globally accessible functions (you can call them at any time like "window.functionName();")                                      
 */
...
function updateHash(){
  ...
  if (hashLink) {
    //find a slide
    if ( $('.slide[data-name="' +hashLink+ '"], .slide[id="' +hashLink+ '"]').length > 0 ){
      //asking for the slide?
      var requestedElement = $('.slide[data-name="' +hashLink+ '"], .slide[id="' +hashLink+ '"]');
  ...
  // Scroll debug mode on
  if(window.scrollDebug || window.location.href.indexOf("slidesScrollDebug")!=-1){
    if($("#slidesScrollDebug").length==0){
      $("body").append('<div class="black padding-1 fixed position-top position-left position-right font-size-12" style="z-index:9999;" id="slidesScrollDebug"></div>');
    }
    window.scrollDebug = 1;
  }
...
function showSlide(requested){
  ...
  if (currenSlideIndex !== requested && window.setStageClasses != 0){
    ...
    if(window.inEditor){ // fix for the Slides app editor
      newSlide.closest("[data-component-cid]").prevAll("[data-component-cid]").find('.slide').addClass('before').removeClass('after');
      newSlide.closest("[data-component-cid]").nextAll("[data-component-cid]").find('.slide').addClass('after').removeClass('before');
    }
...
function changeSlide(n){
  ...
  $html.removeClass("slideDownAllowed");
  ...
  $('.slide').each(function(index, element) {
    ...
    if (window.setStageClasses != 0) {
      if (halfOnScreen && allowToSelect) {
        ...
        if(window.inEditor){ // fix for the Slides app editor
          $element.closest("[data-component-cid]").prevAll("[data-component-cid]").find('.slide').addClass('before').removeClass('after');
          $element.closest("[data-component-cid]").nextAll("[data-component-cid]").find('.slide').addClass('after').removeClass('before');
        }
      ...
      if ((scale > -1 && scale < 1)) {
        ...
        if ($element.hasClass('parallax') || $element.find('.parallax-element')){
          ...
          $element.find('.parallax-element').each(function() {
            var $el = $(this),
                velocity = parseInt($el.attr('data-parallax-velocity')) ? parseInt($el.attr('data-parallax-velocity')) : 50,
...
function playSlideAnimation(slide){
  if(!slide){slide = $(".slide.selected");}
  if (window.isAnimated == "auto") {
    //clearTimeout(window.clearElementAnimation);
    if(!window.cleanupDelaySet && window.setAnimFromCSS){
      for(var i=1; i<10; i++){
        if($(".ae-"+i).length>0 && $(".ae-"+(i+1)).length>0){
          window.cleanupDelay = 1000*(parseFloat($(".ae-"+(i+1)).css("transition-delay")) - parseFloat($(".ae-"+i).css("transition-delay"))); //ms
          window.cleanupDelay = window.cleanupDelay * 9 + 10; // cause we have .ae-1 ... .ae-10 class maximum and .ae-1 has no delay. So max delay is $delay: ($i - 1) * $default-delay-step + $start-delay; (check framework.scss)
          break;
        }
      }
      window.cleanupDelaySet = true;
    }
    if(!window.effectSpeedSet && window.setAnimFromCSS){
      if($(".page-loaded .slide.active:eq(0)").length>0){
        window.slideSpeed = parseFloat($(".page-loaded .slide.active:eq(0)").css("transition-duration"))*1000;
        window.effectSpeed = window.slideSpeed;
        window.effectOffset = window.cleanupDelay - window.effectSpeed;
      }
      window.effectSpeedSet = true;
    }
    window.clearElementAnimation = setTimeout(function(){
      slide.find("[class*='ae-']").addClass('done');
  ...
  if (window.isAnimated == "animateOnEvent") {
    ...
    if (!window.preload) {
      ...
      $("[class*='ae-']").each(function(i, element) {
        ...
        if (isElementInView($ae)) {
          $ae.addClass("do").one('webkitAnimationEnd oanimationend msAnimationEnd animationend', function(){
            if(!window.inEditor){ // if we're not in the Slides app editor 
...
// usage: runAnimation(".slide:eq(2)").then(function(){alert("animation is played");});
window.runAnimation = async function(slide){ // replay animation on the screen in slide
  var promise = new Promise((resolve, reject) => {
    var duration = 0;
    slide = $(slide);
    if(slide.length==0){
      slide = $(".slide.selected"); // currently visible slide
    }
    //updateScroll(); // trigger animation start
    playSlideAnimation(slide); // trigger animation start
    slide.find("[class*='ae-']:not(.done)").each(function(){ // find the longest animation
      duration = Math.max(duration,parseFloat($(this).css("transition-duration")) + parseFloat($(this).css("transition-delay")));
    });
    setTimeout(function(){
      resolve("runAnimation"); // callback when elements transitions finished
    },duration*1000);
  });
  return await promise;
}

// usage: resetAnimation(".someEl").then(function(){alert("animation reset finished");});
window.resetAnimation = async function(el,immediately){ // reset animation of an element and all animatable elements in it
  el = $(el);
  var promise = new Promise((resolve, reject) => {
    var elements = el.add(el.find("[class*='ae-']"));
    elements.removeClass("done");
    var duration = 0;
    var _closest = el.closest(".animate, .active");
    window.resetAnimationClassesBackup = _closest.attr("class") ? _closest.attr("class") : ""; 
    if(immediately){
      _closest.removeClass("animate active");
    }else{
      elements.each(function(){ // find the longest animation
        duration = Math.max(duration,parseFloat($(this).css("transition-duration")) + parseFloat($(this).css("transition-delay")));
      });
      _closest.removeClass("active");
    }
    setTimeout(function(){
      //_closest.addClass("animate");
      _closest.addClass(window.resetAnimationClassesBackup);
      resolve("resetAnimation"); // callback when elements transitions finished
    },duration*1000);
  });
  return await promise;
}
    ...
    if ( $(navigation).length > 0 ){
      $(navigation).each(function(index, element) {
        ...
        var $selectedSlide = $('.slide.selected'),
            parentSlide = parseInt($selectedSlide.attr('data-parent-slide-id')),
...
async function animatePopup(element, isAnimated) {
...
function showPopup(id, focusOn){
  ...
  if (element.length > 0) {
    ...
    animatePopup(element, isAnimated).then(function( status ) { // done
      if(focusOnID){
    ...
    if ( iframes.length > 0  ) {
      iframes.each(function(){
        var iframe = $(this),
            iframeSrc = iframe.attr('src') ? iframe.attr('src') : iframe.attr('data-src'),
...
// Set equal height for elements inside the container
  ...
  if (gridEl.length) {
    $(gridEl).each(function(index, element) {
      ...
      var screenWidth = window.windowWidth,
          ...
          collapseWidth = $(element).attr('data-equal-collapse-width') ? parseInt($(element).attr('data-equal-collapse-width')) : collapseWidth,
...
function sliderStopAndPlayVideos($slider,clickOnControl){
  ...
  if($slider.closest(".popup").length==0 || clickOnControl){ // if slider is not in popup
    ...
    if ( iframe.length > 0  ) {
        var iframeSrc = iframe.attr('src') ? iframe.attr('src') : iframe.attr('data-src'),
            symbol = (iframeSrc.indexOf('?') > -1) ? "&" : "?";
            if(iframeSrc.indexOf("autoplay")==-1){
              iframe.attr("allow","autoplay");
              iframe.attr('src',iframeSrc + symbol + "autoplay=1");
...
function showDropdown($this, $isHover){
  ...
  var offset = $this.offset(),
      ...
      dropdownID = $this.attr('data-dropdown-id'),
      $element = $('.dropdown[data-dropdown-id="' + dropdownID + '"]'),
      elementPosition = $this.attr('data-dropdown-position') ? $this.attr('data-dropdown-position') : $element.attr('class'),
  ...
  window.dropdownTrigger = $this;
  $(window).trigger('dropdownShown',[dropdownID,$this]);
}

// Hides any dropdown
function hideDropdown(id){
  id = typeof id !== 'undefined' ? id : false;
  if(window.debug) console.log("%c hideDropdown("+id+")","color:SteelBlue");
  //hide
  if ($html.hasClass("dropdownShown")){
    hideDropdownOnScrollDelay = 0;
...
function initSliders(){
  var sliderEl = $(document).find('.slider');
  ...
  if ($(sliderEl).length > 0) {
    ...
    $(sliderEl).each(function(index, element) {
      ...
      // check status
      var $el = $(element),
          sliderID = $el.attr('data-slider-id'),
      ...
      if ($el.hasClass('autoplay')) {
        ...
        var interval = ($el.attr('data-slider-interval')) ? parseInt($el.attr('data-slider-interval')) : 5000;
        ...
        if ($el.attr('data-slider-stoponclick')) {
      ...
      if ($el.hasClass('clickable') || $el.hasClass('autoplay') || $el.hasClass('swipeable')){
        ...
        $el.on('next', function(event){
          ...
          var $el = $(this),
              sliderID = $el.attr('data-slider-id'),
              $slider = $(document).find('.slider[data-slider-id="'+sliderID+'"]'),
              $selectedSlides = $(),
              clickTarget = event.target;

          // break
          if($(clickTarget).attr('data-slider-event') == "cancel") return;
          ...
          $slider.each(function(){
            ...
            var $el = $(this),
                ...
                $controller = $(document).find('.controller[data-slider-id="'+sliderID+'"]'),
                $sliderCounter = $(document).find('.sliderCounter[data-slider-id="'+sliderID+'"]'),
            ...
            $selectedSlides = $selectedSlides.add($nextElement);
            ...
            if (sliderID && $controller.length > 0){
              $controller.each(function(){
                var $controller = $(this);
                var controllerSelector = $controller.attr('data-controller-selector') ? $controller.attr('data-controller-selector') : "li";

                $controller.children('.selected').removeClass('selected');
                $controller.children(controllerSelector+':eq('+nextIndex+')').addClass('selected');
              });
            }
          ...
          // set a trigger
          $(window).trigger('sliderChanged',[sliderID,$selectedSlides,$(this)]);
        });
        ...
        $el.on('prev', function(event){
          ...
          $slider.each(function(){
            ...
            $selectedSlides = $selectedSlides.add($prevElement);
  ...
  // controller
  var $controllerEl = $(document).find('.controller');

  if ($controllerEl.length > 0) {
    $($controllerEl).each(function() {
      var $controller = $(this);
      var controllerSelector = $controller.attr('data-controller-selector') ? $controller.attr('data-controller-selector') : "li";
      ...
      $controller.on('click', controllerSelector, function(){
        var $controllerElement = $(this),
            ...
            sliderID = $controller.attr('data-slider-id'),
            $slider = $(document).find('.slider[data-slider-id="'+sliderID+'"]'),
            $nextSlide,
            $selectedSlides = $(),
            $controllers = $(document).find('.controller[data-slider-id="'+sliderID+'"]'),
            $sliderCounter = $(document).find('.sliderCounter[data-slider-id="'+sliderID+'"]');
        ...
        if (!$controllerElement.hasClass('selected')){
          ...
          $slider.each(function(){
            ...
            var $nextSlide = $slider.children('li').eq(nextIndex);
            $nextSlide.removeClass('hide').addClass('selected');
            $selectedSlides = $selectedSlides.add($nextSlide);
          ...
          // set a trigger
          $(window).trigger('sliderChanged',[sliderID,$selectedSlides,$controllerElement]);
        }
      });
    });
  }
...
window.ajaxFormSuccess = function(element){
  var $ajaxForm = element ? $(element) : $(this),
      $ajaxFormButton = $($ajaxForm).find('[data-success-text], [type="submit"], button').first(),
      ajaxFormButtonIsInput = $ajaxFormButton.is('input') ? true : false,
      successText = $ajaxFormButton.attr('data-success-text') ? $ajaxFormButton.attr('data-success-text') : "Done!",
      successClass = $ajaxFormButton.attr('data-success-class') ? $ajaxFormButton.attr('data-success-class') : "green",
...
$(document).ready(function() { "use strict";
  ...
  if($.cookie("slidesScrollDebug")) window.scrollDebug = 1;
 ...
 */
  ...
  $('.nextSlide').on('click', function(){
    if(!window.inEditor){ // disable in the Slides app editor
 ...
 */
  ...
  // we need to use new "wheel" event instead of "mousewheel". Mousewheel causes "glitching" while scrolling scrollable area on Windows.
  ...
  $('body').on(scrollSupportedEvents+' touchend', async function(event){
    ...
    var $currentSection = $('.slide.selected .content').length>0 ? $('.slide.selected .content') : $('.slide.selected'),
        ...
        //deviceZoom = window.frameWidthToDetectZoom ? window.frameWidthToDetectZoom / window.document.documentElement.clientWidth : window.outerWidth / window.document.documentElement.clientWidth, // not used anymore
        minScrollToSlide = (window.isFirefox && window.isWindows) ? 200 : window.minScrollToSlide;

    // in case user removed '.container' from section
    if(isNaN(currentSectionHeight)){
      currentSectionHeight = Math.floor($currentSection.outerHeight());
    }
    ...
    if ( !window.isScroll && !(window.isMobile && window.isSimplifiedMobile)){
      ...
      if(window.scrollDebug){
        $("#slidesScrollDebug").html(`
           screen=`+window.windowWidth+`:`+window.windowHeight+`; 
           slideHeight=`+currentSectionHeight+`; 
           scrollsize:minToSlide=`+scrollsize+`:`+minScrollToSlide+`; 
      ...
      if ((currentSectionHeight > window.windowHeight)){
        ...
        // wait reaching the end of a scroll for correct calculations (for touch devices only)
        if(event.type=="touchend" && typeof document.onscrollend!="undefined" && scrollDirection === "down" && !$html.hasClass("slideDownAllowed")){
          if(window.scrollDebug) $("#slidesScrollDebug").append('awaitScrollEnd=1; ');
          await function () {
            return new Promise((resolve) => {
              $currentSection.one("scrollend",function(){
                resolve();
              });
            });
          }() // execute function
        }
        var sectionScrollBottom = Math.ceil($currentSection.scrollTop() + window.windowHeight);
        var sectionBottom = Math.floor(currentSectionHeight);

        if(window.scrollDebug) $("#slidesScrollDebug").append(`sectionScrollBottom:sectionBottom=`+sectionScrollBottom+`:`+sectionBottom+`; `);
        if ((( scrollDirection === "up" ) && ( $currentSection.scrollTop() <= 0 )) || (( scrollDirection === "down" ) && ( sectionScrollBottom >= sectionBottom ))){
          if(scrollDirection === "down" && !$html.hasClass("slideDownAllowed")){
            if(window.isSafari && window.isMobile){ // mobile Safari is so unique browser... 
              $html.addClass("slideDownAllowed");
              window.allowSlide = 1;
            }else{
              window.allowSlide = 0;
              if(window.scrollDebug) $("#slidesScrollDebug").append(`awaitToSlideDownTimeout set; allowSlide=`+window.allowSlide+`; `);
              window.awaitToSlideDownTimeout = setTimeout(function(){
                $html.addClass("slideDownAllowed");
                window.allowSlide = 1;
                if(window.scrollDebug) $("#slidesScrollDebug").append(`awaitToSlideDownTimeout FINISHED; allowSlide=`+window.allowSlide+`; `);
              },window.awaitToSlideDown);
            }
          }else{
            window.allowSlide = 1;
          }
        ...
        if (window.panelsToHide) { // TO DO: Seems like this condition will never be executed (cause no window.panelsToHide definition)
          ...
          if (scrollDirection === "down" && $currentSection.scrollTop() + window.windowHeight >= Math.floor(currentSectionHeight)) {
      ...
      // end of scroll oversized content
      }
      ...
      if(window.scrollDebug) $("#slidesScrollDebug").append(`collectScrolls=`+Math.abs(window.collectScrolls)+`; allowSlide=`+window.allowSlide+`; `);
    ...
    // end of scroll/mousewheel/touchend event
  });
    ...
    if ($(navigation).is(':empty')) {
      ...
      $(navigation).each(function(index, element) {
        for (var i = 1; i <= slidesNumber; i++){
          ...
          // Add Tooltips
          var title = $('.slide:not(.exclude):eq('+(i - 1)+')').attr('data-title');
          if (title === undefined) {
...
*/
  ...
  $(document).on('click', '.sidebarTrigger[data-sidebar-id]', function(){
    if(window.isEditorOn){return true;} // if we're not in the Slides app editor 
...
*/
  ...
  $(document).on('click', '.popupTrigger[data-popup-id]', function(){
    if(window.isEditorOn){return true;} // if we're not in the Slides app editor 
    var popupID   = $(this).attr('data-popup-id'),
        focusOnID = $(this).attr('data-popup-focus-input'); // ID of input to focus on after showing popup
...
*/
  ...
  $(document).on('click', '[data-slider-action]', function(){
    ...
    if ($(this).attr('data-slider-id')){
      var $this = $(this),
          ...
          sliderID = $this.attr('data-slider-id'),
          action = $this.attr('data-slider-action'),
          $sliders = $(document).find('.slider[data-slider-id="' + sliderID + '"]'),
          $controllers = $(document).find('.controller[data-slider-id="'+sliderID+'"]'),
          $sliderCounter = $(document).find('.sliderCounter[data-slider-id="'+sliderID+'"]');
      ...
      $sliders.each(function(){
        var $slider = $(this),
            $controller = $(document).find('.controller[data-slider-id="'+sliderID+'"]'),
            controllerSelector = $controller.attr('data-controller-selector') ? $controller.attr('data-controller-selector') : "li",
        ...
        } else if (parseInt(action) || parseInt(action)===0 || action === 0 ) {
  ...
  $(window).on('click resize load ready next',function(){
    $(document).trigger('autoHeightRecount');
  });

  $(document).on('autoHeightRecount', function(event){
    $(document).find('[data-slider-id].autoHeight').each(function(index, element) {
      var totalHeight = 0,
  ...
  $(document).on('click', ".slider.clickable[data-slider-id], .controller[data-slider-id]", function(event){
    if($(event.target).attr('data-slider-event') != "cancel") $(window).resize();
  });
...
*/
  ...
  $(document).on('click', '.dropdownTrigger', function(){ // click
    if(window.isEditorOn){return true;} // if we're not in the Slides app editor 
    showDropdown($(this));
  }).on('mouseover', '.dropdownTrigger.hover', function(){ // hover
    showDropdown($(this), "hover");
  });

  // remove on resize
  $(window).on('resize',function(){
    if(!window.preventHideDropdownOnResize){
      hideDropdown();
    }
  });
  ...
  $.fn.slidesDialog = function( options ) {
    ...
    function bindOpenCloseMethods(el){
      $(el).click(function(){
        ...
        }else if(action="open" && id && !window.isEditorOn){
...
*/
  ...
  // Submit data to other services (Integrations tab in the app)
  $('[data-integrations] form').bind('submit',function(){
    var form = $(this);
    var integrations = [];
    var skip = form.attr("data-skip-integrations");
        skip = skip ? JSON.parse(skip) : [];
    $.each(Object.values(JSON.parse($('[data-integrations]').attr('data-integrations'))), function(){
      if(Object.values(skip).indexOf(this)==-1){
        integrations.push(this);
      }
    });
    var data = form.serializeArray();
    data.push({
      name: "dm_integrations",
      value:integrations
    });
    if(form.attr("data-make-custom-webhook")){
      data.push({
        name: "custom_make_url",
        value:form.attr("data-make-custom-webhook")
      });
    }
    if(form.attr("data-zapier-custom-webhook")){
      data.push({
        name: "custom_zapier_url",
        value:form.attr("data-zapier-custom-webhook")
      });
    }
    if(form.attr("data-pabbly-custom-webhook")){
      data.push({
        name: "custom_pabbly_url",
        value:form.attr("data-pabbly-custom-webhook")
      });
    }
    if(form.attr("data-airtable-custom-base-id")){
      data.push({
        name: "custom_airtable_base_id",
        value:form.attr("data-airtable-custom-base-id")
      });
    }
    if(form.attr("data-airtable-custom-table-id")){
      data.push({
        name: "custom_airtable_table_id",
        value:form.attr("data-airtable-custom-table-id")
      });
    }
    $.ajax({
      url: (window.slidesIntegrationsUrl ? window.slidesIntegrationsUrl : 'https://designmodo.com/slides/app/integrations/')+$("[data-dm-pid]").attr("data-dm-pid")+"/",
      method: 'POST',
      cache: false,
      crossDomain: true,
      data: data,
      success:function(data){
        ...
        if(data.status=="error"){
          console.error("Integration Error! Designmodo response: ", data);
        }else{ // success
          console.log("Sent to all integrations: ",data);
        }
      },
      error:function(jqXHR, textStatus, errorThrown){
        console.error("Integration Error!", jqXHR, textStatus, errorThrown);
      }
    });
  });

  // Basic ajax forms submission
  $('#contact-form, [data-ajax-form]').each(function(index, element) {
    $(element).ajaxForm(function() {
      ajaxFormSuccess(element);
    });
  });
...
*/
  ...
  $('audio[data-sound-id]').each(function(event, element){
    var $element = $(element),
        musicID = $element.attr('data-sound-id'),
        audio = $element[0],
  ...
  $('.soundTrigger').click(function(){
    var musicID = $(this).attr('data-sound-id'),
        $audio = $('audio[data-sound-id="'+musicID+'"]'),
        action = $audio.attr('data-sound-action') ? $audio.attr('data-sound-action') : "toggle",
        fade = (parseInt($audio.attr('data-sound-fade')) >= 0 || $audio.attr('data-sound-fade')) ? parseInt($audio.attr('data-sound-fade')) : 500;
